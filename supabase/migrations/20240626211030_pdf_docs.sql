...
CREATE TABLE IF NOT EXISTS public.pdf_docs
(
...
CREATE TRIGGER set_pdf_docs_timestamp
    BEFORE INSERT OR UPDATE ON public.pdf_docs
    FOR EACH ROW
EXECUTE PROCEDURE basejump.trigger_set_timestamps();

-- protect the updated_by and created_by columns by setting them to be read-only and managed by a trigger
CREATE TRIGGER set_pdf_docs_user_tracking
    BEFORE INSERT OR UPDATE ON public.pdf_docs
    FOR EACH ROW
...
----------------
-- create policy "All logged in users can select" on public.pdf_docs
--     for select
--     to authenticated
--     using (true);
...
----------------
-- create policy "All authenticated and anonymous users can select" on public.pdf_docs
--     for select
...
----------------
-- create policy "Account members can insert" on public.pdf_docs
--     for insert
--     to authenticated
--     with check (
--     (account_id IN ( SELECT basejump.get_accounts_with_role()))
...
---------------
-- create policy "Account members can update" on public.pdf_docs
--     for update
--     to authenticated
--     using (
--     (account_id IN ( SELECT basejump.get_accounts_with_role()))
...
----------------
-- create policy "Account members can delete" on public.pdf_docs
--     for delete
--     to authenticated
--     using (
--     (account_id IN ( SELECT basejump.get_accounts_with_role()))
...
----------------
-- create policy "Account owners can delete" on public.pdf_docs
--     for delete
--     to authenticated
--     using (
